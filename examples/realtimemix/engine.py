...
class AudioEngine:
    ...
    def __init__(self, sample_rate: int = 48000, buffer_size: int = 1024, 
                 ...
                 enable_streaming: bool = True, streaming_threshold_mb: int = 100):
        """
        初始化音频引擎
        ...
        self.tracks: Dict[str, npt.NDArray[np.float32]] = {}  # Store original audio data (for preloaded tracks)
        ...
        logger.info(f"Streaming mode: {'enabled' if enable_streaming else 'disabled'} (threshold: {streaming_threshold_mb}MB)")
    ...
    def _load_large_file_streaming(self, track_id: str, file_path: str, speed: float,
        ...
        try:
            with sf.SoundFile(file_path) as f:
                ...
                # 内存管理优化
                effective_memory_limit = min(self.max_memory_usage * 2, estimated_size)  # 允许更大的内存使用
                merge_threshold = effective_memory_limit * 0.8  # 80%时才合并
                
                logger.info(f"使用内存限制: {effective_memory_limit/(1024*1024):.1f}MB, 合并阈值: {merge_threshold/(1024*1024):.1f}MB")
                ...
                if chunks:
                    ...
                    logger.info(f"大文件加载完成: {track_id} ({len(final_data)}帧，{final_data.nbytes/(1024*1024):.1f}MB)")
                    if on_complete:
                        on_complete(track_id, True)
                else:
            ...
            if track_id in self.track_states:
                ...
                if state.get('streaming_mode', False) and track_id in self.streaming_tracks:
                    ...
                    def seek_callback(success):
                        if success:
                            logger.debug(f"Streaming seek completed for {track_id} to {position_sec:.2f}s")
                        else:
        ...
        try:
            ...
            with self.lock:
                if track_id in self.tracks:
                    ...
                    sample_rate = self.track_states.get(track_id, {}).get('sample_rate', self.sample_rate)
                ...
                if len(mono_data) > 0:
                    ...
                    logger.debug(f"RMS loudness for {track_id}: {rms:.4f} (analyzed {len(mono_data)} samples)")
                    return float(rms)
                else:
            ...
            for track_id in active_tracks:
                try:
                    ...
                    if state.get('streaming_mode', False) and track_id in self.streaming_tracks:
                        ...
                        streaming_track = self.streaming_tracks[track_id]
                        try:
                            chunk = self._get_streaming_audio_with_padding(track_id, streaming_track, state, frames)
                        except Exception as e:
                        ...
                        # 处理循环播放
                        if state.get('loop', False) and self._is_streaming_track_at_end(streaming_track, state):
                        ...
                        try:
                            # 检查是否为unsampled模式，如果是则跳过重采样
                            if state.get('unsampled_mode', False):
                                # unsampled模式：直接提取音频数据，不进行任何重采样
                                chunk, new_position = self._extract_audio_chunk_unsampled(
                    ...
                    if chunk is not None and chunk.shape[0] > 0:
                        ...
                        if chunk.shape[1] != self.channels:
                            logger.warning(f"Track {track_id} channel mismatch: {chunk.shape[1]} vs {self.channels}")
                            continue
                        ...
                        if chunk.shape[0] == frames:
                                                    ...
                                                    if std_diff > mean_diff * 3:
                                                        ...
                                                        for i in range(1, len(channel_data) - 1):
                                                            if diff[i-1] > mean_diff + 2 * std_diff:
                                                                # 检测到电流声，用邻近样本平均值替代
                                                                smoothed[i] = (channel_data[i-1] + channel_data[i+1]) * 0.5
                            ...
                            if min_frames > 0:
                                ...
                                if track_sample_rate != self.sample_rate and min_frames > 0:
                                    ...
                                    if not is_track_muted and track_volume > 0.001:
                                        ...
                                        mixed_64 = mix_part.astype(np.float64) + chunk_part.astype(np.float64) * (0.92 if abs(track_sample_rate - 24000) < 100 else 1.0)
                                        mixed_max = np.max(np.abs(mixed_64))
                                        if mixed_max > 0.98:
                                            mixed_64 *= (0.98 / mixed_max)
                                    ...
                                    if not is_track_muted and track_volume > 0.001:
                                        ...
                                        if track_volume != 1.0:
                                            chunk_part = chunk_part * track_volume
                                        np.add(mix_buffer[:min_frames], chunk_part, out=mix_buffer[:min_frames])
                                ...
                                if min_frames < frames:
                                    ...
                                    if fade_length > 0:
                                        ...
                                        # 对24kHz轨道使用更保守的淡出
                                        track_sample_rate = state.get('sample_rate', self.sample_rate)
                                        start_fade = 0.3 if abs(track_sample_rate - 24000) < 100 else 0.5
                                        
                                        # 创建更短的淡出序列
                                        fade_out = np.linspace(start_fade, 0.0, fade_length)[:, np.newaxis]
                                        ...
                                        if actual_fade_length > 0:
                                            np.add(mix_buffer[min_frames:end_pos], fade_chunk[:actual_fade_length], 
                            ...
                            if track_sample_rate != self.sample_rate:
                                ...
                                if not is_track_muted and track_volume > 0.001:
                                    ...
                                    mixed_64 = mix_buffer.astype(np.float64) + truncated_chunk.astype(np.float64) * (0.92 if abs(track_sample_rate - 24000) < 100 else 1.0)
                                    mixed_max = np.max(np.abs(mixed_64))
                                    if mixed_max > 0.98:
                                        mixed_64 *= (0.98 / mixed_max)
    ...
    def _resample_chunk_realtime(self, chunk: npt.NDArray, rate_ratio: float, target_frames: int) -> npt.NDArray:
                    ...
                    if target_frames > 4:
                        ...
                        for channel in range(self.channels):
                            ...
                            for i in range(1, target_frames - 1):
                                if i % 2 == 1:  # 只对插值点进行平滑
                                    smoothed[i] = (upsampled[i-1, channel] + upsampled[i, channel] + upsampled[i+1, channel]) / 3
        ...
        try:
            ...
            if chunk.shape[0] > 1:
                ...
                for channel in range(self.channels):
                    # 进行高质量线性插值
                    resampled[:, channel] = np.interp(source_indices, 
                    ...
                    for channel in range(self.channels):
                        ...
                        for i in range(1, target_frames - 1):
                            if diff[i-1] > spike_threshold:
                                # 检测到噪音尖峰，用邻近样本的平均值替代
                                resampled[i, channel] = (resampled[i-1, channel] + resampled[i+1, channel]) * 0.5
        ...
        with self.lock:
            ...
            if state.get('streaming_mode', False) and track_id in self.streaming_tracks:
                ...
                return {
                    ...
                    'streaming_mode': True,
                    'unsampled_mode': state.get('unsampled_mode', False),  # 添加unsampled模式标识
    ...
    def get_track_count(self) -> Dict[str, int]:
        ...
        with self.lock:
            ...
            muted = len([t for t, s in self.track_states.items()
    ...
    def set_streaming_config(self, enable_streaming: bool = None, 
                           threshold_mb: int = None) -> Dict[str, Any]:
        """
        设置流式播放配置
    ...
    def _get_streaming_audio_with_padding(self, track_id: str, streaming_track: StreamingTrackData, 
                                        state: Dict[str, Any], frames: int) -> npt.NDArray:
        """
        获取带静音填充的流式音频数据
        ...
        # 计算总的虚拟长度（包含静音填充）
        total_virtual_frames = padding_start + int(streaming_track.duration * streaming_track.engine_sample_rate) + padding_end
        ...
        if start_virtual < total_virtual_frames - padding_end and output_offset < frames:
            ...
            audio_frames_needed = min(frames - output_offset, 
                                    (total_virtual_frames - padding_end) - start_virtual)
    ...
    def _is_streaming_track_at_end(self, streaming_track: StreamingTrackData, state: Dict[str, Any]) -> bool:
        """检查流式轨道是否到达末尾（考虑静音填充）"""
        padding_end = state.get('padding_frames_end', 0)
        virtual_position = state.get('virtual_position', 0)
        total_virtual_frames = (state.get('padding_frames_start', 0) + 
                              int(streaming_track.duration * streaming_track.engine_sample_rate) + 
                              padding_end)
        
        return virtual_position >= total_virtual_frames
    
    def _is_streaming_track_finished(self, streaming_track: StreamingTrackData, state: Dict[str, Any]) -> bool:
        """检查流式轨道是否完全结束（不循环时）"""
        return (self._is_streaming_track_at_end(streaming_track, state) and 
                not state.get('loop', False))
    
    def _reset_streaming_track_for_loop(self, streaming_track: StreamingTrackData, state: Dict[str, Any]) -> None:
        """重置流式轨道用于循环播放"""
                    ...
                    for channel in range(self.channels):
                        if channel < len(last_sample.flatten()):
                            ...
                            processed_chunk[:smooth_length, channel] = (
                                alpha * transition + (1 - alpha) * processed_chunk[:smooth_length, channel]
                            )
                        ...
                        if idx < chunk.shape[0] - 1:
                            ...
                            if smooth_range > 0:
                                ...
                                processed_chunk[idx:idx + smooth_range + 1, channel] = (
                                    alpha * transition + (1 - alpha) * processed_chunk[idx:idx + smooth_range + 1, channel]
                                )
    ...
    def _load_streaming_track_unsampled(self, track_id: str, file_path: str, auto_normalize: bool,
        ...
        try:
            ...
            streaming_track = StreamingTrackData(
                ...
                engine_channels=self.channels,
                buffer_seconds=15.0,  # 15秒缓冲
                unsampled_mode=True  # 启用unsampled模式，跳过重采样
            )
    ...
    def _extract_audio_chunk_unsampled(self, audio_data: npt.NDArray, position: int, 
                                      speed: float, loop: bool, frames: int) -> Tuple[Optional[npt.NDArray], int]:
        """
        专门为unsampled模式设计的音频块提取函数 - 不进行任何重采样操作
        ...
        Returns:
            (音频块, 新位置) 或 (None, position) 如果轨道结束
        """
        # 计算需要读取的帧数（基于播放速度）
        read_frames = int(frames * speed)
        remaining = len(audio_data) - position
        ...
        # 直接提取音频数据，不进行重采样
        actual_read_frames = min(read_frames, remaining)
        chunk = audio_data[position:position+actual_read_frames].copy()
        new_position = position + actual_read_frames
        
        # 处理循环播放
        if actual_read_frames < read_frames and loop:
            loop_frames_needed = read_frames - actual_read_frames
            if loop_frames_needed <= len(audio_data):
                loop_chunk = audio_data[:loop_frames_needed]
                chunk = np.concatenate((chunk, loop_chunk))
                new_position = loop_frames_needed
        ...
        # unsampled模式：根据音频引擎的需要调整输出大小，但不重采样
        if chunk.shape[0] != frames:
            if chunk.shape[0] > frames:
                # 如果读取的数据太多，直接截取
                chunk = chunk[:frames]
                ...
                # 如果读取的数据不足，用零填充
                padded_chunk = np.zeros((frames, self.channels), dtype=np.float32)
